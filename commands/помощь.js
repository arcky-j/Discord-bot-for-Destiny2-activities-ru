...
module.exports = {
        ...
        .addStringOption(option => 
            option.setName('команда')
                ...
                .addChoices(
                    ...
                    {name: '/подземелье', value: 'dung'},
                    {name: '/сбор_настройка', value: 'custom'},
    ...
    async execute(interaction) {
        ...
        //сообщение строится из нескольких переменных
        //не лучшая идея весь этот текст держать в переменных, но пока так; команд ещё не слишком много
        ...
        const cmd1 = 'Команда для сбора команды в рейды. Создаёт сообщение с карточкой сбора. Карточка содержит: название рейда, требования и заметку от лидера (если есть), время и дату начала рейда, готовых пойти Стражей, Стражей в резерве и ID для взаимодействия с карточкой. Присутствуют кнопки для вступления в боевую группу и резерв, а также выхода из них.\nЗа 10 минут до начала рейда все участники получат уведомление в личные сообщения. Если в боевой группе нет 6 Стражей, уведомление получают и резервисты.\nРейд можно отменить, перенести и передать другому игроку, если вы лидер группы.\n';
        ...
        const cmd3 = 'Команда для сбора команды куда угодно - это вы определяете сами. Создаёт сообщение с карточкой сбора. Созданное сообщение содержит: название активности, требования и заметку от лидера (если есть), время и дату начала активности, готовых пойти Стражей, Стражей в резерве и ID для взаимодействия. Присутствуют кнопки для вступления в боевую группу и резерв, а также выхода из них.\nЗа 10 минут до начала активности все участники получат уведомление в личные сообщения. Если в боевой группе нет указанного количества Стражей, уведомление получают и резервисты.\nАктивность можно отменить, перенести и передать другому игроку, если вы лидер группы.\n';
        ...
        switch (command){
            case 'raid': embed.addFields({name: '**/рейд {название рейда} {время начала рейда по МСК} {дата рейда} [требования к участникам] [заметка от лидера сбора]**', value:cmd1});
                break;
            case 'dung': embed.addFields({name: '**/подземелье {название подземелья} {время начала подземелья по МСК} {дата подземелья} [требования к участникам] [заметка от лидера сбора]**', value:cmd2});
                break;
